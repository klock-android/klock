...
void CallGraphPass::initCallSites(){
		if( Ctx->CallSites.empty() ){
			for( CalleeMap::iterator i = Ctx->Callees.begin(), 
					e = Ctx->Callees.end(); i!=e; ++i ){
				FuncSet &CEEs = 	i->second;
				CallInst* CI = i->first;
				for (FuncSet::iterator ii = CEEs.begin(), ee = CEEs.end(); ii != ee; ++ii) {
					if( Ctx->CallSites.find(*ii) == Ctx->CallSites.end()) {
						// the function is seen for the first time 
						CISet tmp;
						tmp.insert(CI);
						Ctx->CallSites[*ii] = tmp;
					} else{
						Ctx->CallSites[*ii].insert(CI);
					}
				}
			}
		}
